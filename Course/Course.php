<?php
    ...
    $sql = "SELECT * FROM course JOIN chapter ON (course.CourseID = chapter.CourseID) WHERE course.CourseID = $cID;";
    $sql2 = "SELECT * FROM course WHERE class = $class";
    $Chemistry_Chapter = $conn->query($sql);
    $CourseRN = $conn->query($sql);
    $pages = $conn->query($sql2);
    $page_course =  $conn->query($sql2);
    $rtrn = $conn->query($sql2);
...
<body>
    ...
    <div class="container py-6">
        <h1 class="mb-4">Class: <?php echo $course['Class'];?></h1>
        <a href="#" class="nav-linked dropdown-toggle upscale-20 section-title-custom" data-bs-toggle="dropdown">COURSE</a>
        <div class="dropdown-menu container-btn fade-down m-0">
            <?php
            while($page= mysqli_fetch_assoc($pages) ){
                ?>
                <a href="../Course/Course.php?cls=<?php echo $page['Class']?>&v=<?php echo $page['CourseID']?>" class="dropdown-item"><?php echo $page["CourseName"];?></a>
            <?php
            }
            ?>
        </div>
    ...
    <div class="container">
        <div class="row g-4">
            <h1 class="mb-4"><?php echo $course["CourseName"];?></h1>
        </div>
            <?php
            while($row = mysqli_fetch_assoc($Chemistry_Chapter) ){
            ?>
                <a href="../Chapter/Chapter.php?cls=<?php echo $class?>&ch=<?php echo $row["ChapterID"];?>&crs=<?php echo $row["CourseID"];?>" class="btn btn-white btn-lg-course py-3 px-4 mt-2"><?php echo $row["ChapterName"];?></a>
            <?php
                }
                $return = mysqli_fetch_assoc($rtrn);
            ?>
             <div class="py-4">
                <a class="btn btn-white btn-lg-return upscale-30 px-4 py-1" href="../Courses/listcourse.php?v=<?php echo $return['Class']?>">< Return</a>
            </div>
    </div>
