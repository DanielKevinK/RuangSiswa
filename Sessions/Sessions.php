<?php
    ...
    $sql = "SELECT course.CourseName, course.CourseName, chapter.ChapterName, sesi.sessionName FROM course
    JOIN chapter ON (course.CourseID = chapter.CourseID)
    JOIN sesi ON (chapter.ChapterID = sesi.ChapterID)
    JOIN ppt on (sesi.sessionID = ppt.sessionID AND sesi.ChapterID = ppt.ChapterID)
    WHERE ppt.ChapterID = $chapter AND ppt.SessionID = $session;";

    $sql1 = "SELECT course.CourseName, course.CourseName, chapter.ChapterName, sesi.sessionName FROM course
    JOIN chapter ON (course.CourseID = chapter.CourseID)
    JOIN sesi ON (chapter.ChapterID = sesi.ChapterID)
    JOIN video on (sesi.sessionID = video.sessionID AND sesi.ChapterID = video.ChapterID)
    WHERE video.ChapterID = $chapter AND video.SessionID = $session;";
    ...
    $chsi = $conn->query($sql2);
    $chsi1 = $conn->query($sql2);
?>
...
<body>
                ...
                <div class="col-lg-10 text-center">
                    ...
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb justify-content-center">
                            ...
                            <?php
                            $course = mysqli_fetch_assoc($CourseRN)
                            ?>
                            <li class="breadcrumb-item text-white active" aria-current="page"><?php echo $course["CourseName"];?></li>
                        </ol>
                    </nav>
                </div>
            </div>
    ...
    <div class="container">
        <?php
         $session = mysqli_fetch_assoc($sessions)
         ?>
            <div class="row g-4">
                     <h1 class="mb-4"><?php echo $session["sessionName"];?></h1>
            </div>

            <div class="row g-4">
                     <h1 class="mb-4">PPT:</h1>
            </div>
        
            <?php
                $i = 1;
                while($row = mysqli_fetch_assoc($chsi) ){
            
            ?>
                    <a href="../PPT/Powerpoint.php?ch=<?php echo $row["ChapterID"];?>&ssi=<?php echo $row["sessionID"];?>" class="btn btn-white btn-lg-course py-3 px-4 mt-2">PPT <?php echo $i++;?></a>
            <?php
                }
            ?>
            <div class="row g-4">
                <h1 class="mb-4 py-6">Video:</h1>
            </div>
            <?php
                $j = 1;
                while($row1 = mysqli_fetch_assoc($chsi1) ){
            ?>
                    <a href="../Video/VideoPlayer.php?ch=<?php echo $row1["ChapterID"];?>&ssi=<?php echo $row1["sessionID"];?>" class="btn btn-white btn-lg-course py-3 px-4 mt-2">Video <?php echo $j++;?></a>
            <?php
                }
            ?>
            <?php
